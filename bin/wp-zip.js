...
const zipScript  = path.join( scriptDir, 'zip.sh' )
...
if (isYarn && isWindows) {
	...
	console.log('   For best results, use: npm run zip')
	console.log('')
}

const child = spawn( 'bash', [ zipScript, ...process.argv.slice(2) ], {
	stdio: 'inherit',
	cwd: process.cwd(),
	env: {
		...process.env,
